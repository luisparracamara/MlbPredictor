...
public class ComparacionPitcherServiceImpl implements ComparacionPitcherService {
    ...
    public List<PitcherResponse> compararPitchers(ComparacionPitcherRequest comparacionPitcherRequest) {
        ...
        Document callPitcherLocal = baseballReference
                .getHtml("https://www.baseball-reference.com/players/gl.fcgi?id="
                        + comparacionPitcherRequest.getPitcherLocal() + "&t=p&year=" + comparacionPitcherRequest.getDate().getYear());
        Document callPitcherVisitante = baseballReference
                .getHtml("https://www.baseball-reference.com/players/gl.fcgi?id="
                        + comparacionPitcherRequest.getPitcherVisitante() + "&t=p&year=" + comparacionPitcherRequest.getDate().getYear());

        return Arrays.asList(responsePitcher(callPitcherLocal, comparacionPitcherRequest.getDate()),
                responsePitcher(callPitcherVisitante, comparacionPitcherRequest.getDate()));
    }

    private PitcherResponse responsePitcher(Document pitcherData, LocalDate date) {

        List<PitcherResultados> resultadosPitcher = obtenerResultadosPitcher(pitcherData, date);
    ...
    private List<PitcherResultados> obtenerResultadosPitcher(Document call, LocalDate date) {
        ...
        pitcherData.eachText().forEach(game -> {
            ...
            if (!pitcherResultados[0].equals("May") && !pitcherResultados[0].equals("Jun")
                ...
                if (resultados.get(resultados.size() - 1).getFecha().isAfter(date) && !resultados.isEmpty()) {
                    resultados.remove(resultados.size() - 1);
                }
            }
        });

//        log.debug("[PitcherResultados {} ]", resultados);
        return resultados;
    }
    ...
    private Pitcher obtenerPitcherData(List<PitcherResultados> pitcherResultados) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Error al extraer datos de pitcher: " + pitcherResultados);
        }
